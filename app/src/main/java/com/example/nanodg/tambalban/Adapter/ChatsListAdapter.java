...
public class ChatsListAdapter extends RecyclerView.Adapter<ChatsListAdapter.ViewHolder> implements Filterable {
    ...
    public void onBindViewHolder(ViewHolder holder, @SuppressLint("RecyclerView") final int position) {
        ...
        if (c.getSender().getKey().equals(set.readSetting("myid"))) {
            holder.title.setText(c.getReceiver().getUsername());

        }
        else if(c.getReceiver().getKey().equals(set.readSetting("myid")))
        {
            holder.title.setText(c.getSender().getUsername());
    ...
    private class ItemFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            for (int i = 0; i < list.size(); i++) {
                String str_title = list.get(i).getReceiver().getUsername();
